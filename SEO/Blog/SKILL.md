...
description: >-
  SEO/GEO 博客正文生产与质检规则（90 篇 pillar 集群）。写/改/审 article.md、
  11 项发布门禁、EEAT、反 AI 模板、关键词密度或外链内链时使用。
---

# SEO Blog Content Skill

> 适用范围：`SEO/Blog/pillar1` … `pillar8` 下所有 `article.md`（及同批次 legacy 单篇博客若用户指定）。  
> **技能库入口**：[`Skills/seo-geo-claude-skills-main/build/seo-blog-content/SKILL.md`](../../Skills/seo-geo-claude-skills-main/build/seo-blog-content/SKILL.md)
...
| pillar2 | `014-code-agent-vs-data-agent` |
...
完整质检契约（SEO 技术、EEAT、反 AI 模板、字数/密度、修复工作流、报告格式）见 **[`content-quality-gates.md`](./content-quality-gates.md)**。

### 发布门禁 · 11+1 项（90/90）

| # | 脚本 | 检验 |
|---|------|------|
| 1–9 | 关键词占位、title/desc、meta 堆砌、大纲、内链、外链位置、高 DR、重合度、HTTP | 见 [`SKILL.md`](./SKILL.md) 各硬规则节 |
| 10 | `audit-content-quality.py` | EEAT 信号、重复句、AI 模板句、标题关键词堆砌、内联外链 ≥5 |
| 11 | `audit-wordcount.py` | 字数 **1900–2800**；密度按关键词词数自适应（1–3 词 0.6–1.8%；4–5 词 0.35–1.5%；6+ 词 0.2–1.0%） |
| + | `audit-eeat.py` | CORE-EEAT 快扫 12/12（FAQ ≥4 问、schema 作者等） |

**反模板零容忍**：Pilot/Operational note 填充句、跨篇 filler H2、verbatim 重复 citation 句、`this workflow` 占位 — 见 [`content-quality-gates.md` §3](./content-quality-gates.md)。

## 质检命令（发布前必跑）

```bash
# 11 项主门禁 + EEAT 快扫（详见 content-quality-gates.md）
...
**通过标准：** 上述 **11 项 + `audit-eeat.py`** 均为 **90/90 Pass**；外链重合度 **0 violations / 4005 pairs**。

## 与其他规则的关系

- 字数与密度以 [`audit-wordcount.py`](./audit-wordcount.py) 自适应规则为准（**非**固定 1.2–1.7%）
- 内容深度、EEAT、反 AI 模板见 [`content-quality-gates.md`](./content-quality-gates.md)
