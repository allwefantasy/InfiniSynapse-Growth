...
- **Hero cover** for blog bundles: first image shows exact H1, 1200×630, dark tech abstract geometry, HTML-rendered title — see [blog-hero-cover-spec.md](blog-hero-cover-spec.md)
...
## 9. Generate Preview HTML (Blog Bundles)

When delivering a multi-article SEO blog bundle (`article.md` + `meta-tags.html` + `schema.json` per folder):

1. Run `build-preview.py` (or equivalent) to produce `{folder}/preview.html` for each article.
2. Emit `{pillar}/INDEX-preview.html` linking all previews.
3. Confirm LOCAL PREVIEW banner, hero/body images load, and CMS metadata lines are stripped from the body.

Full rules: [blog-preview-html-spec.md](blog-preview-html-spec.md). Preview files are **not** deployed to production.
