...
## ⚠️ 必读：线上当前为什么还报错（实测诊断）

我们抓取了线上页面 `https://infinisynapse.com/en/blog/thoughtspot-vs-databricks-genie` 的原始 HTML，结果：

| `<head>` 标签 | 线上实测 | 我们交付文件 | 结论 |
|---|---|---|---|
| `<title>` | ✅ 有（但 71 字符超长） | ✅ 已改 40–60 | 已注入，需用新值 |
| `<meta name="description">` | ⚠️ 有，但是**站点默认中文标语**（非本文描述） | ✅ 每篇 150–160 | **没读取每篇描述** |
| `rel="canonical"` | ❌ **完全没有** | ✅ 每篇都有 | **没注入** |
| `og:*` | ❌ **完全没有** | ✅ 每篇都有 | **没注入** |
| `twitter:*` | ❌ **完全没有** | ✅ 每篇都有 | **没注入** |
| `ld+json` | ✅ 有 | ✅ | 已注入 |
| `<h1>` | ✅ 1 个 | ✅ | 正确 |

**根因：部署时只注入了「标题 + schema」，没有注入每篇的 canonical / description / og / twitter，页面用的是站点默认 `<head>`。**

→ 我们改源文件无法自动反映到线上。**必须由部署代码把每篇 `head.html`（或 `seo-meta.json` 字段）注入页面 `<head>`。** 这是当前红叉的唯一解。

### 最小修复（二选一）

**A. 整段注入 head.html（最省事）**
```
对每篇 /en/blog/{slug}：把 content/.../head.html 全文插入 <head>
```
`head.html` 已含 canonical + description + og + twitter + JSON-LD，且 `<title>` 已是 40–60。

**B. 用 seo-meta.json 程序化注入**（见下方 Next.js 示例）——至少注入：`canonical`、`meta_description`、`og.*`、`twitter.*`、`title`。

修复后 QuickCreator 的 Canonical / Meta Description / Social Media / Meta Title 四项即转绿。
...
| Canonical 缺失 | 每篇 `head.html` / `seo-meta.json` 已含 canonical（`https://infinisynapse.com/en/blog/{slug}`） |
| 社交标签缺失 | 每篇 `head.html` / `seo-meta.json` 已含完整 OG + Twitter |
| **Meta Title 不在 40–60** | **97 篇已改为 40–60 字符 `<title>`（保留关键词）；3 篇关键词本身 >60，保留完整关键词（QC 黄色警告，可接受）** |
...
```jsonc
{
  ...
  "url": "https://infinisynapse.com/en/blog/julius-ai-alternatives",
