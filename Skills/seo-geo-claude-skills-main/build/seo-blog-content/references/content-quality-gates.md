...
> **活文档入口**：[`SKILL.md`](./infinisynapse-blog-full-rules.md)（外链/内链/关键词/大纲硬规则）+ 本文（内容深度、EEAT、反 AI 模板、发布门禁）。  
...
**密度 · 按关键词词数自适应**（**新集群 21–25 下限统一 ≥1.2%**，1.19% 及以下 Fail；16–20 既有文按历史 ≥1.0% 兼容）：

| Target keyword 词数 | 可接受密度（新集群 21–25） |
|---------------------|------------|
| 1–3 词 | **1.2%** – 1.8% |
| 4–5 词 | **1.2%** – 1.6% |
| 6+ 词 | **1.2%** – 1.4% |

> **硬规则**：Pillar 21–25 及此后新文章关键词密度须 **≥1.2%**（自查目标）。`audit-wordcount.py` 全局硬 Fail 线仍为 1.0%（防 16–20 旧文回归），因此新文须在写作/复核时确认已达 1.2%。Vibe 系列（16–20）正文可 weave 规划表**核心词**（不含 ` reddit`），按 1.0% 下限兼容。

**分布要求**：

- 首段（TL;DR / Key Definition）须出现至少 1 次完整 Target keyword
- H2/H3 中含关键词的标题 **≤2 个**（≥3 个 → 标题堆砌 Fail）
- 禁止用 `this workflow` 等无搜索量代词替换主关键词（见 [`SKILL.md` § Target keyword](./infinisynapse-blog-full-rules.md)）
...
见 [`SKILL.md` § 大纲结构](./infinisynapse-blog-full-rules.md)。摘要：
...
外链、内链、重合度、高 DR 规则见 [`SKILL.md`](./infinisynapse-blog-full-rules.md)。内容质量附加要求：
...
- Cluster guides 表格或内链锚文本带部署序号（如 `[002 Data Agent Manifesto]`、`002 Data Agent Manifesto |`）→ Fail；见 [`SKILL.md` § 主题集群内链](./infinisynapse-blog-full-rules.md)
...
grep -Erl '\[[0-9]{3} ' pillar*/**/article.md | wc -l                 # 0（无序号内链锚文本）
```
...
硬规则全文：[`SKILL.md`](./infinisynapse-blog-full-rules.md) · Cursor 规则：`.cursor/rules/seo-blog-high-dr-citations.mdc`
