#!/usr/bin/env bash
# Batch AI hero images for Pillars 21-25 via OpenOctopus CLI (ooct).
#
# Prereq:
#   python3 "$SCRIPT_DIR/gen-hero-prompts-p21-25.py"
#   ooct auth login
#
# Usage:
#   bash "$SCRIPT_DIR/gen-hero-images-ai-p21-25.sh"              # all 88
#   bash "$SCRIPT_DIR/gen-hero-images-ai-p21-25.sh" pillar21     # one pillar
#   bash "$SCRIPT_DIR/gen-hero-images-ai-p21-25.sh" --dry-run    # plan only
#
set -euo pipefail
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    pillar2[1-5]*) PILLAR_FILTER="$1"; shift ;;
...
articles=()
for pillar in "$BLOG"/pillar2[1-5]-*; do
  [[ -d "$pillar" ]] || continue
