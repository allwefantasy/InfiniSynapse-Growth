...
def strip_body_h1(md: str) -> str:
    """Remove the leading H1 so the deploy body has no H1 (page <h1> = title)."""
...
- URL 格式：`https://infinisynapse.com/en/blog/{{slug}}`（英文站，无日期、无尾斜杠）
- 站内相对路径：`/en/blog/{{slug}}`
- 示例：`/en/blog/julius-ai-alternatives`
- 中文版（如启用）：`/zh/blog/{{slug}}`
...
- **正文已无 H1**：页面 `<h1>` 用 `meta-tags.html` 的 `<title>`（或 catalog 的「英文标题」）渲染，避免双 H1
- 所有 `##` / `###` 标题需生成 `id`（TOC 锚点跳转）
- 内链已是 `/en/blog/other-slug` 相对路径
...
| 在正文再加一个 `# H1` | 双 H1，On-Page 报错 |
| 给站内 `/en/blog/` 链接加 nofollow | 破坏内链 |
