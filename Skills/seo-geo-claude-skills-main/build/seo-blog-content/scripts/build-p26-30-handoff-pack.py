#!/usr/bin/env python3
"""Build programmer handoff pack for Pillar 26–30 (100 governance→viz articles)."""
...
OUT = BLOG / "p26-30-handoff-pack"
ARTS = OUT / "articles"
PILLARS = (
    sorted(BLOG.glob("pillar2[6-9]-*"))
    + sorted(BLOG.glob("pillar30-*"))
)
...
def main() -> None:
    ...
    (OUT / "seo-meta-p26-30.json").write_text(
        json.dumps({"count": len(seo_subset), "articles": seo_subset}, ensure_ascii=False, indent=2)
        + "\n",
        encoding="utf-8",
    )

    for name in ("sitemap.xml", "pillar26-30-topic-cluster-architecture_20260715.md"):
        src = BLOG / name
        if src.is_file():
            shutil.copy2(src, OUT / name)

    readme = f"""# Pillar 26–30 · 治理 / MDM / 工程 / 仓湖 / 可视化 · 程序员交付包（{len(rows)} 篇）

新发 **100 篇** SEO 博客（Pillar 26–30，编号 388–487）。包内已含更新后的 **`sitemap.xml`**（在现有线上 sitemap 基础上合并本批次 URL）。
...
p26-30-handoff-pack/
├── README.md
├── deploy-manifest.csv              ← 100 篇部署清单
├── seo-meta-p26-30.json             ← slug / URL / 标题索引
├── sitemap.xml                      ← 全站 sitemap（含新文章，替换线上）
├── pillar26-30-topic-cluster-architecture_20260715.md
└── articles/
    └── pillar{{26-30}}-{{theme}}/
        └── {{编号}}-{{slug}}/
            ...
            ├── meta-tags.html
            ├── schema.json
            └── images/                ← hero + og-cover + 正文数据图（chart-*.png）
```
...
1. **`deploy-manifest.csv`** — 按 `完整URL` 在 CMS 创建 100 个新路由。
...
4. **`images/`** — 上传 hero（`hero-*.png` 1200×630）、`og-cover.png` 及正文数据图（`chart-*.png`）。
5. **`sitemap.xml`** — 替换 `https://infinisynapse.com/sitemap.xml`（全量已合并本批次 URL）。
...
```bash
curl -sI "https://infinisynapse.com/en/blog/data-governance-frameworks" | head -5
curl -s "https://infinisynapse.com/en/blog/data-governance-frameworks" | grep -i canonical
```
"""
    (OUT / "README.md").write_text(readme, encoding="utf-8")

    zip_path = BLOG / "p26-30-handoff-pack.zip"
    if zip_path.exists():
        zip_path.unlink()
    ...
    missing_head = sum(1 for r in rows if not (OUT / r["交付目录"] / "head.html").is_file())
    print(f"Missing head.html in pack: {missing_head}")
